...
export default class Validation {
  ...
  private composeValidator: (rule: ValidationRule, msg: string) => Validator = (rule, msg) => val => {
    console.log('Validator', rule(val))
    return !rule(val) ? msg : '';
  };
  ...
  private validatePhone: ValidationRule = val => /^([0-9\(\)\/\+ \-]*)$/.test(val);
  ...
  isUsername: MakeValidator = (msg = 'Can contain only letters, numbers, underscores or hyphens') => this.composeValidator(this.validateUsername, msg);
  ...
  isPassword: MakeValidator = (msg, type: PassTypes) => {
    return this.composeValidator(this.validatePassword(type), msg || 'Password must not contain whitespaces');
  }
  ...
  validateField = (field: HTMLInputElement, rules: Array<Validator>): string => {
    const label: string = field.nextElementSibling.textContent;
